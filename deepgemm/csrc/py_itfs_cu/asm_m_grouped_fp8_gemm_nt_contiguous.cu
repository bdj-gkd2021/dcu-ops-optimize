...
void _m_grouped_fp8_gemm_nt_contig_asm_impl(
    torch::Tensor input,
    ...
    d_prob->a = (const char*)b_qweight.data_ptr();
    d_prob->b = (const char*)input.data_ptr();
    ...
    sprintf(func_name_fp, "DeepGemm_W8A8_F8_PERCHANNEL_ASM_TN_MT%dX%dX%d_FP16", BLOCKM, BLOCKN, BLOCKK);
    sprintf(func_name_bf, "DeepGemm_W8A8_F8_PERCHANNEL_ASM_TN_MT%dX%dX%d_BF16", BLOCKM, BLOCKN, BLOCKK);
    sprintf(fp16_co_file_tn, "DeepGemm_W8A8_F8_MARLIN_PERCHANNEL_ASM_TN_MT%dX%dX%d_FP16.co", BLOCKM, BLOCKN, BLOCKK);
    sprintf(bf16_co_file_tn, "DeepGemm_W8A8_F8_MARLIN_PERCHANNEL_ASM_TN_MT%dX%dX%d_BF16.co", BLOCKM, BLOCKN, BLOCKK);
...
torch::Tensor m_grouped_fp8_gemm_nt_contiguous(
  ...
  int mode) {
    ...
    if (mode == 1000) {
        ...
        output.scalar_type(), "_m_grouped_fp8_gemm_nt_contig_asm_impl",[&] {
            _m_grouped_fp8_gemm_nt_contig_asm_impl<256, 256, 128, scalar_t>(
        ...
        at::ScalarType::BFloat16,
        output.scalar_type(), "_m_grouped_fp8_gemm_nt_contig_asm_impl",[&] {
            _m_grouped_fp8_gemm_nt_contig_asm_impl<256, 128, 128, scalar_t, 512>(
            input, b_qweight, output, a_scale, b_scale, m_indices, experts_num);
        }
        );
    } else {
