...
class DeviceRuntime {
    ...
    std::shared_ptr<hipDeviceProp_t> cached_prop;
...
public:
    ...
    std::shared_ptr<hipDeviceProp_t> get_prop() {
        if (cached_prop == nullptr) {
            ...
            cached_prop = std::make_shared<hipDeviceProp_t>();
            result = hipGetDeviceProperties(cached_prop.get(), device_idx);
            if (result != hipSuccess) {
    ...
    void set_num_sms(const int& new_num_sms) {
        LIGHTOP_HOST_ASSERT(0 <= new_num_sms and new_num_sms <= get_prop()->multiProcessorCount);
        num_sms = new_num_sms;
    }
    ...
    void set_tc_util(const int& new_tc_util) {
        LIGHTOP_HOST_ASSERT(0 <= new_tc_util and new_tc_util <= 100);
        tc_util = new_tc_util;
    }
