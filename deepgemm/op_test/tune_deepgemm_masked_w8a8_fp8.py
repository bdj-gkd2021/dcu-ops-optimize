...
from deepgemm import m_grouped_fp8_gemm_nt_masked_impl
...
def generate_test_data(E: int, m: int, n: int, k: int, 
    ...
    for j in range(E):
        masked_m[j] = torch.randint(int(expected_m_per_group*0.7), int(expected_m_per_group*1.3), (1,)).item()
    ...
    return input, input_scale, weight, weight_scale, masked_m
...
def benchmark_single_mode(mode: int,
    ...
    try:
        ...
        for _ in range(warmup):
            m_grouped_fp8_gemm_nt_masked_impl(
                (q_input, a_scale),
        ...
        cost = triton.testing.do_bench(
            lambda: m_grouped_fp8_gemm_nt_masked_impl(
                (q_input, a_scale),
                (weight_marlin, w_scale),
                output,
                masked_m,
...
def tune_for_expected_m(E: int, m: int, n: int, k: int,
    ...
    # 生成测试数据
    q_input, a_scale, weight, weight_scale, masked_m = generate_test_data(
    ...
    weight_marlin = weight8bit_nt_kpack2_marlin(weight)
    ...
    # 计算参考输出
    if verify:
        ref_output = native_fp8_perChannel_batch_matmul(
            q_input, weight, a_scale, weight_scale, out_dtype)
        # valid_m = int(expected_m_per_group * 0.7)
        # ref_output[:, valid_m:, :] = 0
    
    # 获取所有可用 modes
    cuda_modes = [i for i in range(85)]         # 不过lds
    cuda_modes1 = [i for i in range(100, 185)]  # bypass kernel
    ...
    all_modes = cuda_modes + cuda_modes1 + asm_modes
